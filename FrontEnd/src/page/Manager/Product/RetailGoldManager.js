...
const RetailGoldManager = () => {
    ...
    const getProduct = async () => {
        try {
            ...
            const res = await axios.get(
                `https://jssatsproject.azurewebsites.net/api/product/getall?categoryID=5&stallid=${stallDetail}&pageIndex=${currentPage}&pageSize=${pageSize}&ascending=${ascending}&includeNullStalls=false`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
    ...
    const handleSearch = async () => {
        try {
            ...
            const res = await axios.get(
                `https://jssatsproject.azurewebsites.net/api/product/search?categoryID=5&stallid=${stallDetail}&searchTerm=${searchQuery}&pageIndex=${currentPage}&pageSize=${pageSize}&ascending=${ascending}&includeNullStalls=false`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
