...
const SearchInvoice = () => {
  ...
  const filteredinvoice = listInvoice.filter((invoice) =>
    (invoice.id.toString().includes(searchTerm) ||
     invoice.first_name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    ...
    <div className="w-[800px] overflow-hidden">
      <table className="font-inter w-full table-auto border-separate border-spacing-y-1 overflow-y-scroll text-left md:overflow-auto">
        ...
        <tbody>
          {filteredinvoice && filteredinvoice.length > 0 && filteredinvoice.map((item, index) => {
            return (
              <tr
                    key={index}
                  ...
                  >
                    ...
                    <td className=" text-sm font-normal text-[#637381]">{item.first_name}</td>
