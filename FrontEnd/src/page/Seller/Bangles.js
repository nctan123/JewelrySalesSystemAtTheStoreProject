  ...
  const getRingSearch = async (searchTerm, page) => {
    try {
      const res = await axios.get(
        `https://jssatsproject.azurewebsites.net/api/Product/Search?categoryId=3&searchTerm=${searchTerm}&pageIndex=${page}&pageSize=10`
      );
...
<<<<<<< HEAD
  ...
  const filteredBangles = listBangles.filter((bangles) =>
    (bangles.id.toString().includes(searchTerm) ||
  bangles.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  bangles.code.toLowerCase().includes(searchTerm.toLowerCase()))
    );
...
<<<<<<< HEAD
      <div className='h-[88vh] overflow-y-auto mt-3 flex justify-center'>
        <div className='grid grid-cols-4 mt-1 w-[850px]'>
          {filteredBangles && filteredBangles.length > 0 &&
            filteredBangles.filter(item => item.categoryId === 3 && item.status === "active").map((item, index) => {
              return (
                <div key={`bangles-${index}`} class="relative flex flex-col justify-center items-center w-[200px] px-[20px] pb-8 h-[280px] bg-[#fff] shadow-xl rounded-lg mb-2">
                  <div className=' bg-[#fff] rounded-md shadow-md'>
                    <img class="mt-0 w-28 h-28  rounded-lg hover:-translate-y-30 duration-700 hover:scale-125" src={bangles} />
                    </div>
      ...
      <div className='h-[80vh] overflow-y-auto mt-3 flex-col justify-center mx-auto'>
        <div className='grid grid-cols-4 mt-1 w-fit space-x-2 mx-auto'>
          {listRing && listRing.length > 0 &&
            listRing.filter(item => item.categoryId === 3).map((item, index) => {
              return (
