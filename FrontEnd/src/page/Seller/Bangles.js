        ...
        <div className='grid grid-cols-5 mt-1 w-[800px]'>
          {filteredBangles && filteredBangles.length > 0 &&
            filteredBangles.filter(item => item.categoryId === 3 && item.status === "active").map((item, index) => {
              return (
                <div key={`bangles-${index}`} className={clsx(style.card)} onClick={() => dispatch(addProduct(item))} >
                  <img className=' mt-0 w-[100%] h-[70%] rounded-xl object-cover bg-[#ffffff1f]' src={bangles} />
