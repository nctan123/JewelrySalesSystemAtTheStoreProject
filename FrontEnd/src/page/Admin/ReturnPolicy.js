...
const ReturnPolicy = () => {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen">
            <div>
                <h1 className="text-2xl font-bold text-center mb-4">Return Buy Back Policy List</h1>

                <div className="w-[1000px] overflow-hidden">
                    <div className='float-left pl-4'>
                        <button onClick={() => setIsCreateModalOpen(true)}>Add new buy back policy</button>
                    </div>
                        ...
                        <tbody>
                            {currentPolicys.map((item, index) => (
                                <tr key={index} className="cursor-pointer bg-[#f6f8fa] drop-shadow-[0_0_10px_rgba(34,46,58,0.02)] hover:shadow-2xl">
                                    ...
                                    <td className="text-sm font-normal text-[#637381]">{getNamefromDescription(item.description)}</td>
                                    <td className="text-sm font-normal text-[#637381]">{formatEffectiveDate(item.effectiveDate)}</td>
            ...
            {selectedPolicy && !isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-10 bg-gray-800 bg-opacity-50">
                    <div className="bg-white rounded-lg p-8 max-w-md w-full">
                        ...
                        <p className="text-sm text-gray-700 mb-2 text-xl"><strong>Description:</strong> {getNamefromDescription(selectedPolicy.description)}</p>
                        <p className="text-sm text-gray-700 mb-2 leading-relaxed break-words">
                            {getDescription(selectedPolicy.description)}
                        </p>
